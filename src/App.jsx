...
const generateDemoData = () => {
  ...
  const apexRatings = {
    ...
    fin_rate: createRating(3, "Average day rate ÃÂ£1,800"),
    // People
    ...
    srv_size: createRating(3, "Average deal size ÃÂ£320k"),
    ...
    cli_size: createRating(3, "42% from ÃÂ£250k+ clients"),
    ...
    // Market Profile
    mkt_size: createRating(3, "Large addressable market ÃÂ£2bn+"),
  ...
  const techbridgeRatings = {
    ...
    fin_rate: createRating(2, "Day rate ÃÂ£1,150"),
    ...
    srv_size: createRating(2, "Average deal ÃÂ£85k"),
  ...
  const phoenixRatings = {
    ...
    fin_rate: createRating(1.5, "Day rate ÃÂ£850"),
    ...
    srv_size: createRating(1.5, "ÃÂ£40k average deal"),
        ...
        <div className="bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
          ...
          <div className={`px-4 py-3 ${isAllComplete ? 'bg-gradient-to-r from-green-500 to-emerald-500' : 'bg-gradient-to-r from-blue-600 to-indigo-600'} text-white`}>
            <div className="flex items-center justify-between">
              ...
              {isAllComplete && <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full">Ã¢ÂÂ Complete!</span>}
            </div>
          </div>
...
function BenchmarkComparison({ scores }) {
  ...
  const comparisonData = FRAMEWORK.themes.map(t => ({
    name: t.name.length > 12 ? t.name.substring(0, 11) + 'â¦' : t.name,
  ...
  return (
    <div className="bg-white rounded-lg border border-blue-200 p-4 mb-4 shadow-lg">
      ...
      <button onClick={onClose} className="mt-3 text-xs text-gray-500 hover:text-gray-700">
        Or start with blank assessment Ã¢ÂÂ
      </button>
    </div>
  );
}
